...
int UkkonenTree::findChildIndex(Node* parent, int c){
     ...
     if (parent->children.count(f)) return c+1;
     f.firstCharacter=-1;
     if (parent->children.count(f)) return c+1;
     return -1;
}
...
UkkonenTree::Edge UkkonenTree::findEdge(Node* parent, int i){
     ...
     auto t=parent->children.find(f);
     if (t!=parent->children.end()) return *t;
     f.firstCharacter=-1;
     t=parent->children.find(f);
     if (t!=parent->children.end()) return *t;
     return f; //Lol, not found
}
...
void UkkonenTree::addEdge(Node* parent, Edge& edge, int index){
    ...
    if (index>=0) parent->children.erase(edge);
    parent->children.insert(edge);
}
